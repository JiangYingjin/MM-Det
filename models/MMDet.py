...
from LLaVA.llava.model.builder import (
    load_pretrained_model as load_llava_pretrained_model,
)
...
class MMEncoder(nn.Module):
    ...
    def get_prompt(self):
        if self.conv_mode == "llava_v1":
            ...
            raise ValueError(
                f'Expect conv mode in "llava_v1", "mistral". Get {self.conv_mode}.'
            )
        return text
        ...
        with torch.inference_mode():
            ...
            if isinstance(x, torch.Tensor):
                ...
                try:
                    ...
                    raise ValueError(
                        f"The video length {x.size(1)} is longer than the maximum interval {self.interval} for a single inference. Please divide input into more short clips."
                    )
